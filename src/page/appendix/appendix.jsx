...
const Appendix = () => {
  ...
  const columns = appendixConst.map(item => ({
    title: item.title,
  ...
  const [dataSource, setDataSource] = useState(appendixDataSource.map((item, index) => ({
    ...item,
    key: index,
  })));
  ...
  const handleEditOk = () => {
    setDataSource(prevDataSource => prevDataSource.map(item => item.key === data.key ? { ...item, ...data } : item));
